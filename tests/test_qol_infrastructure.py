...
class TestCIWorkflows:
    ...
    def test_ci_has_lint_job(self):
        ...
        assert "ruff" in content.lower(), \
            "ci.yml powinien zawierać krok ruff (linting)"
    ...
    def test_ci_has_coverage_xml(self):
        ...
        assert "coverage.xml" in content, \
            "ci.yml powinien generować coverage.xml dla Codecov"

    def test_ci_has_type_check_job(self):
        content = self._read_workflow("ci.yml")
        assert "mypy" in content, \
            "ci.yml powinien zawierać job type-check z mypy"
    ...
    def test_dependabot_covers_pip(self):
        content = self._read_workflow("../../.github/dependabot.yml")  # special path
        # Try direct path
        ...
        assert "pip" in dep_content, \
            "dependabot.yml powinien monitorować ekosystem pip"
...
class TestMakefile:
    ...
    def test_makefile_has_lint_ruff(self):
        ...
        assert "ruff" in content, \
            "Makefile powinien używać ruff do lintowania"

    def test_makefile_has_format(self):
        assert "format" in self._makefile_content(), \
            "Makefile powinien zawierać cel format"
    ...
    def test_makefile_strict_test_uses_itdoc_strict(self):
        content = self._makefile_content()
        assert "ITDOC_STRICT=1" in content, \
            "Makefile strict-test powinien ustawiać ITDOC_STRICT=1"
...
class TestDeprecationFixes:
    ...
    def test_no_bare_except_without_variable(self):
        ...
        assert not violations, (
            f"Znaleziono puste excepts bez `as exc` (py/empty-except):\n"
