...
def main():
    ...
    for md_path in md_files:
        ...
        for ordinal, (heading_text, heading_norm, level, heading_path, anchor) in enumerate(headings, start=1):
            if (doc_uid, anchor) in existing:
                continue
            section_uid = make_section_uid(doc_uid, heading_text, ordinal)
            new_rows.append((section_uid, doc_uid, heading_text, heading_norm, level, heading_path, anchor, ordinal))
    ...
    print("\nKluczowe anchory:")
    for anchor in ['standardy-i-compliance', 'raci-i-role', 'zalenoci-dokumentu',
                   'powizania-meta', 'mace-zastosowanie-standardy-i-normy',
