...
def _apply_action_raw(
    ...
    if action == "delete":
        ...
        tag = wrap_tag or ">"
        wrapped_lines = "\n".join(
            f"{tag} {line}" if line.strip() else "" for line in sec_body.strip().splitlines()
        )
...
def undo_session(audit_path: str, session_id: str) -> None:
    ...
    if not records:
        safe_print(
            colored(f"[WARN] Brak zapisanych operacji dla sesji '{session_id}'.", ANSI_YELLOW)
        )
        return
    restored = 0
    for rec in records:
...
def process_file(
    ...
    if interactive and not dry_run:
        ans = (
            input(
                colored(
                    f"  [ZAPYTANIE] Zmodyfikować {md_file.name}? [T/n]: ",
                    ANSI_BOLD,
                )
            )
            .strip()
...
def patch_section(
    ...
    if simulate_jsonl:
        ...
        safe_print(
            colored(
                f"[OK] Symulacja zapisana do: {simulate_jsonl} ({len(ops)} operacji)", ANSI_GREEN
            )
        )
        return {}
    ...
    safe_print(
        f"  Błędy:                  {colored(str(stats['error'] + len(errors)), ANSI_RED) if (stats['error'] + len(errors)) else '0'}"
    )
...
def report_matrix(
    ...
    for f in files:
        try:
            _, body = strip_frontmatter(f.read_text(encoding="utf-8", errors="replace"))
            sections = {m.group(1).strip() for m in header_re.finditer(body)}
