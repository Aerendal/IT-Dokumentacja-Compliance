...
ARCHETYPES = [
    # === INCYDENT / RESPONSE ===
    (
        ['incident response','response plan','reagowanie na incydent','obsługa incydentu','incident handling',
         'response procedure','outage response','breach response','alert response','failure response'],
    ...
    (
        ['disaster recovery','odtwarzanie po awarii','business continuity','ciągłość działania','bcp','drp',
         'recovery plan','business impact','bia'],
    ...
    (
        ['system architecture','architektura systemu','architecture overview','high-level design','hld',
         'architektura platformy','architektura aplikacji','architektura rozwiązania','solution architecture'],
    ...
    # === WYMAGANIA ===
    (
        ['wymagania','requirements specification','software requirements','business requirements',
         'functional requirements','system requirements','specyfikacja wymagań'],
    ...
    # === HARMONOGRAM ===
    (
        ['harmonogram','project schedule','gantt','timeline','roadmap','milestone','kamień milowy'],
    ...
    # === RUNBOOK / SOP ===
    (
        ['runbook','playbook','procedura operacyjna','operational procedure','sop','instrukcja'],
    ...
    # === MONITORING ===
    (
        ['monitoring','alerting','observability','dashb','alert','metryki systemowe','metryki operacyjne'],
    ...
    # === SECURITY POLICY ===
    (
        ['security policy','polityka bezpieczeństwa','information security','isms policy',
         'polityka dostępu','access policy','password policy'],
    ...
    # === REJESTR RYZYK ===
    (
        ['risk register','rejestr ryzyk','risk assessment','ocena ryzyk','risk management',
         'risk log','analiza ryzyka'],
    ...
    (
        ['vulnerability','podatność','cve','pentest','penetration test','security scan',
    ...
    # === API ===
    (
        ['api','openapi','rest','graphql','asyncapi','sdk','endpoint','swagger','api reference',
         'api design','api specification','api documentation'],
    ...
    # === MODEL DANYCH ===
    (
        ['data model','model danych','schema','data dictionary','słownik danych','erd','entity',
    ...
    # === DEPLOYMENT / RELEASE ===
    (
        ['deployment','wdrożenie','release plan','release notes','deployment plan','go-live',
         'rollout','launch plan'],
    ...
    # === SETUP / KONFIGURACJA ===
    (
        ['setup','konfiguracja','configuration guide','installation','instalacja','getting started',
    ...
    # === CHANGE MANAGEMENT ===
    (
        ['change request','wniosek o zmianę','change management','formularz zgłoszenia zmiany',
         'ocena wpływu zmiany','change control','change log'],
    ...
    # === SLA / KPI / METRYKI ===
    (
        ['sla','ola','service level','poziom usług','kpi','metryki jakości','performance metrics',
    ...
    # === COST BENEFIT / TCO ===
    (
        ['cost benefit','koszty i korzyści','analiza kosztów','tco','total cost','roi','cba',
         'analiza finansowa','budżet','planowanie wydatków'],
    ...
    # === AI/ML ===
    (
        ['machine learning','ml model','ai model','neural','llm','generative ai','genai',
         'bias','fairness','algorithm','model card','adversarial','recommendation engine'],
    ...
    # === INTEGRACJE ===
    (
        ['integration','integracja','middleware','event','messaging','kafka','rabbitmq','webhook',
         'event-driven','microservice','service mesh'],
    ...
    # === SCRUM / AGILE ===
    (
        ['sprint','retrospective','retrospektywa','velocity','burndown','burnup','sprint review'],
    ...
    (
        ['backlog','product backlog','epic','feature','priorytetyzacja','prioritization','product roadmap'],
    ...
    # === VENDOR / CONTRACT ===
    (
        ['vendor','dostawc','supplier','contract','umowa','rfp','rfi','rfc','przetarg','oferta'],
    ...
    (
        ['log','rejestr','audit trail','audit log','ścieżka audytowa','event log','activity log'],
    ...
    # === DOKUMENTACJA / GUIDE ===
    (
        ['guide','documentation','developer guide','user guide','technical guide','best practices',
         'wytyczne','przewodnik','instrukcja','handbook'],
...
def main():
    parser = argparse.ArgumentParser(description="Wzbogacaj placeholder szablony o semantyczne opisy")
    parser.add_argument('--dry-run', action='store_true', help="Pokaż co zostałoby zmienione (bez zapisu)")
    parser.add_argument('--limit', type=int, default=0, help="Przetwórz max N dokumentów (0=wszystkie)")
