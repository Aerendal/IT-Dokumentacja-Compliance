...
def apply_tag_rules(cur: sqlite3.Cursor, rules_path: Path, run_dir: Path) -> dict:
    ...
    cur.executemany(
        "INSERT OR IGNORE INTO document_tags_current(document_id, tag_type, tag_value) VALUES(?,?,?)",
        inserts,
    )
...
def _load_snapshot_index(cur: sqlite3.Cursor, snapshot_id: int) -> dict[str, dict]:
    idx: dict[str, dict] = {}
    cur.execute(
        "SELECT path, hash_sha256_v2, template_id FROM documents_snapshot WHERE snapshot_id=?",
        (snapshot_id,),
    )
...
def compute_transitions(
    cur: sqlite3.Cursor, from_snapshot_id: int, to_snapshot_id: int, run_dir: Path
) -> dict:
    ...
    for key in common_keys:
        ...
        if len(removed) == 1 and len(added) == 1:
            kind = (
                "template_id"
                if (idx_from[key]["kind"] == "template_id" or idx_to[key]["kind"] == "template_id")
                else "hash_sha256_v2"
            )
            moves.append(
                {
    ...
    cur.execute(
        "DELETE FROM path_transitions WHERE from_snapshot_id=? AND to_snapshot_id=?",
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["identity_kind", "identity_key", "old_path", "new_path"])
        for m in moves:
...
def _load_snapshot_map(cur: sqlite3.Cursor, snapshot_id: int) -> dict[str, str]:
    ...
    cur.execute(
        "SELECT path, hash_sha256_v2 FROM documents_snapshot WHERE snapshot_id=?", (snapshot_id,)
    )
...
def record_run_metrics(
    ...
    if pr_disk_sha != pipeline_result_sha:
        raise ValueError(
            f"pipeline_result sha mismatch (computed {pipeline_result_sha} vs disk {pr_disk_sha})"
        )
    ...
    hashing_rule_version = pipeline_result.get("dup_content_policy", {}).get(
    ...
    dup_clusters_total = pipeline_result.get("dup_content_policy", {}).get(
        "dup_hash_clusters_total"
    )
    dup_clusters_exempt_only = pipeline_result.get("dup_content_policy", {}).get(
        "dup_hash_clusters_exempt_only"
    )
    dup_clusters_unexpected = pipeline_result.get("dup_content_policy", {}).get(
        "dup_hash_clusters_unexpected"
    )
    ...
    snapshot_status = pipeline_result.get("snapshot", {}).get("action") or pipeline_result.get(
        "snapshot", {}
    ).get("status")
    ...
    """,
        (
            ...
            manifest_hash,
            alignment_hash,
            hashing_rule_version,
            policy_version_dup,
            dup_exemptions_path,
            docs_count,
            anomalies_total,
            dup_clusters_total,
            dup_clusters_exempt_only,
            dup_clusters_unexpected,
            tags_inserted_rows,
            tags_dead_rules,
            tags_conflicts_docs,
            snapshot_status,
            ...
            diff_unchanged,
            moves_detected,
            ambiguous_changed_clusters,
            unchanged_clusters,
            changed_clusters,
            manual_meta_total,
            manual_meta_resolved,
            manual_meta_pass,
            pipeline_result_sha,
        ),
    )
...
def emoji_check(root: Path, run_dir: Path) -> dict:
    ...
    (run_dir / "emoji_report.json").write_text(
...
def ensure_schema(conn: sqlite3.Connection):
    ...
    if not table_has_column(cur, "documents_current", "template_id"):
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_doc_current_template_id ON documents_current(template_id)"
        )
    if not table_has_column(cur, "documents_snapshot", "hash_sha256_v2"):
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_doc_snap_hash_v2 ON documents_snapshot(snapshot_id, hash_sha256_v2)"
        )
    if not table_has_column(cur, "documents_snapshot", "template_id"):
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_doc_snap_template_id ON documents_snapshot(snapshot_id, template_id)"
        )
    # snapshot meta
    meta_cols = [
        ("templates_manifest_hash_v2", "TEXT"),
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_snap_manifest_hash_v2 ON snapshots(templates_manifest_hash_v2)"
    )
    ...
    if "identity_kind" not in cols:
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_transitions_identity_kind ON path_transitions(identity_kind)"
        )
    if "note" not in cols:
...
def coverage_report(run_dir: Path) -> dict:
    ...
    try:
        ...
        if not path_col:
            ...
            (run_dir / "csv_coverage_report.json").write_text(
    ...
    files_without_log = sorted(fs_set - csv_set)
    log_without_files = sorted(csv_set - fs_set)
...
def diagnostics_report(db_path: Path, run_dir: Path) -> dict:
    ...
    with col_csv.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["title_norm", "count", "title", "path", "source"])
        for tn, cnt in norms:
            cur.execute(
                "SELECT title, path, source FROM documents_current WHERE title_norm=? ORDER BY path",
                (tn,),
            )
    ...
    with dup_csv.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["hash_sha256_v2", "count", "path"])
        for h, cnt in dups:
            cur.execute(
                "SELECT path FROM documents_current WHERE hash_sha256_v2=? ORDER BY path", (h,)
            )
...
def build_manifest_v2(run_dir: Path) -> tuple[str, Path]:
    ...
    with out.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["path", "hash_sha256_v2"])
        w.writerows(rows)
    return sha256_bytes(out.read_bytes()), out


def load_counts(cur):
    cur.execute("SELECT COUNT(*) FROM documents_current")
    files_count = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM anomalies_current")
    anomalies_total = cur.fetchone()[0]
    cur.execute(
        "SELECT COUNT(*) FROM (SELECT title_norm FROM documents_current GROUP BY title_norm HAVING COUNT(*)>1)"
    )
    collisions = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM documents_current WHERE hash_sha256_v2 IS NULL")
    null_v2 = cur.fetchone()[0]
...
def build_coverage(cur) -> Coverage:
    ...
    distinct_title_norm = cur.execute(
        "SELECT COUNT(DISTINCT title_norm) FROM documents_current"
    ).fetchone()[0]
    aligned_not_ok = cur.execute(
        "SELECT COUNT(*) FROM documents_current WHERE aligned<>1"
    ).fetchone()[0]
    empty_path = cur.execute(
        "SELECT COUNT(*) FROM documents_current WHERE path IS NULL OR TRIM(path)='';"
    ).fetchone()[0]
    dup_path = 0
    csv_vs_fs_missing = 0
    csv_vs_fs_extra = 0
...
def validate(cur, base_anom: int, base_coll: int, base_duphash: int) -> ValidateReport:
    ...
    status = "PASS"
    cur.execute(
        "SELECT COUNT(*) FROM (SELECT hash_sha256_v2, COUNT(*) c FROM documents_current WHERE hash_sha256_v2 IS NOT NULL GROUP BY hash_sha256_v2 HAVING c>1)"
    )
...
def prune_snapshots(cur) -> PruneReport:
    ...
    keep_ids = set()
    cur.execute(
        "SELECT snapshot_id FROM snapshots ORDER BY snapshot_id DESC LIMIT ?", (RETENTION_WEEKLY,)
    )
    ...
    return PruneReport(sorted(keep_ids), to_delete)
...
def main():
    ...
    if cov_db.aligned_not_ok or cov_db.empty_path:
        print("FAIL: coverage preconditions not met")
        sys.exit(1)
